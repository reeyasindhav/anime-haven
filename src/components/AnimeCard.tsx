...
export function AnimeCard({ anime, index = 0 }: { anime: Anime; index?: number }) {
  ...
  return (
    <article className="group stagger" style={{ animationDelay: `${Math.min(index, 8) * 60}ms` }}>
      <div className="relative overflow-hidden rounded-2xl bg-muted card-lift">
        <Link to="/anime/$animeId" params={{ animeId: anime.id }} className="block">
          ...
          />
          <span className="absolute inset-0 bg-gradient-to-t from-ink/90 via-ink/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
          <span className="absolute inset-x-3 bottom-3 flex items-center justify-between opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 translate-y-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-ink/90 px-2.5 py-1 text-xs font-bold text-ink-foreground backdrop-blur">
